...
- **Providers**: Connectors to external services, such as LLM providers (e.g., AWS Bedrock) and tool providers. Providers are responsible for translating between the framework's internal data model and the external service's API.
...
7. The `AgentLoop` executes any tool calls using the appropriate tool providers.
