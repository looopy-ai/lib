...
async function reconnectionExample() {
	...
	const eventSource1 = new EventSource(
		`http://localhost:3000/sse/stream?contextId=${contextId}`,
	);
...
async function progressTrackingExample() {
	...
	eventSource.addEventListener("task-status", (event: MessageEvent) => {
		...
		if (data.kind === "task-status") {
			...
			if (progress) {
				...
				console.log(
					`[${data.taskId}] Status: ${data.status}${progress.complete ? " ✓" : ""}`,
				);
			}
		}
	});
	...
	setTimeout(() => {
		...
		for (const [taskId, progress] of tasks.entries()) {
			console.log(
				`Task ${taskId}: ${progress.status}, ${progress.toolCalls} tool calls`,
			);
		}
		eventSource.close();
	}, 30000);
